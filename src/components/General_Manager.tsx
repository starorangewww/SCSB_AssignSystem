...
const GeneralManagerDashboard: React.FC = () => {
 ...
 useEffect(() => {
  ...
  // ✅ Step 2. 每次切換「訊息分頁」時讀取最新 localStorage
useEffect(() => {
  // 只在第一次開啟時初始化，不會覆蓋
  initSharedMessages();
  ...
  // ✅ Step 3. 監聽是否有新回覆（區協理回覆 → 總經理紅點提示）
  useEffect(() => {
    const checkForNewReplies = () => {
      const stored = localStorage.getItem("sharedMessages");
      if (stored) {
        const allMessages = JSON.parse(stored);
        const repliedByArea = allMessages.some(
          (msg: any) =>
            msg.replies?.some((r: any) => r.sender.includes("區協理")) &&
            msg.sender.includes("總經理室")
        );
        if (repliedByArea) setHasNotification(true);
      }
    };
    window.addEventListener("storage", checkForNewReplies);
    return () => window.removeEventListener("storage", checkForNewReplies);
  }, []);
  ...
  // ✅ Step 4. 總經理寄信給區協理（含寄件備份）
  const handleSendToAreaManager = () => {
    const newMessage = {
      id: Date.now(),
      sender: user?.username || "總經理室",
      senderRole: "總經理",
      recipientRole: "區協理",
      ownerRole: "總經理", // ✅ 新增歸屬者
      subject: "最新行銷活動指示",
      preview: "請北一區統計分行推廣成效並回報。",
      content: "請於下週三前上傳北一區各分行的行銷成效報表。",
      time: new Date().toLocaleTimeString("zh-TW", {
        hour: "2-digit",
        minute: "2-digit",
      }),
      starred: true,
      box: "寄件備份", // ✅ 改成寄件備份，避免誤分類到收件夾
      replies: [],
    };
    ...
    alert("✅ 郵件已成功寄出，北一區協理將收到新信！");
  };

  // ✅ Step 5. 回覆邏輯
  const handleReplySend = () => {
    if (!replyContent.trim()) return alert("請輸入回覆內容！");
    const currentUser = user?.username || "總經理室";
    ...
    window.dispatchEvent(new Event("storage")); // ✅ 加上這行，確保區協理畫面即時更新
    alert("✅ 回覆已寄出！（北一區協理將會看到這封回覆）");
  };
  ...
  return (
    <div className="relative min-h-screen bg-[#f8f8f8] text-gray-800">
      ...
      <header className="fixed top-0 left-0 w-full z-50 bg-gradient-to-r from-[#D23F06] to-[#E45E06] text-white shadow-md py-3 px-8 flex justify-between items-center">
        {/* <h1 className="text-lg font-semibold tracking-wide">上海商業儲蓄銀行</h1> */}
      ...
      <div className="fixed top-1/3 left-6 h-[45vh] w-14 bg-white rounded-xl shadow-lg border border-gray-200 flex flex-col items-center py-4 space-y-6 overflow-hidden">
        <SideIcon icon={<img src={Home} alt="首頁" className="w-25 h-25"/>} label="首頁" active={activeTab === "all"|| activeTab === "region"} onClick={() => setActiveTab("all")} />
        <SideIcon icon={<img src={Anal} alt="分析" className="w-25 h-25"/>} label="分析" active={activeTab === "analysis"} onClick={() => setActiveTab("analysis")} />
        <SideIcon icon={<img src={Message} alt="訊息" className="w-25 h-25"/>} label="訊息" active={activeTab === "message"} onClick={() => setActiveTab("message")} />
        <SideIcon icon={<img src={Setting} alt="設定" className="w-25 h-25"/>} label="設定" onClick={() => alert("設定功能開發中")} />
        <SideIcon icon={<img src={Logout} alt="登出" className="w-25 h-25"/>} label="登出" onClick={() => setUser(null)} />
      </div>
      ...
      <main className="pt-24 px-28 pb-20">
        {activeTab === "all" || activeTab === "region" ? (
          <>
            <div className="flex justify-between items-center mb-4">
              ...
              <div className="flex flex-col items-end">
                <div className="flex items-center space-x-2">
                  <input type="date" value={dateRange.start} onChange={(e) => handleDateChange("start", e.target.value)} className="border border-gray-300 rounded-md text-sm px-2 py-1 focus:ring-1 focus:ring-orange-400" />
                  <span>至</span>
                  <input type="date" value={dateRange.end} onChange={(e) => handleDateChange("end", e.target.value)} className="border border-gray-300 rounded-md text-sm px-2 py-1 focus:ring-1 focus:ring-orange-400" />
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-2xl shadow-lg border border-gray-200">
              <div className="flex items-center border-b border-gray-100 relative px-6 space-x-10">
                <TabButton label="全行" active={activeTab === "all"} onClick={() => setActiveTab("all")} width="w-28" />
                <TabButton label="區域" active={activeTab === "region"} onClick={() => setActiveTab("region")} width="w-28" />
              </div>
                ...
                {activeTab === "all" ? (
                  <div className="flex flex-col space-y-10">
                    ...
                    <div className="grid grid-cols-4 gap-4 justify-items-center items-center mx-auto max-w-[1240px]">
                      {[
                        { src: "/gm_img/gm_1_1_1.png", alt: "預測貢獻度" },
                        { src: "/gm_img/gm_1_1_2.png", alt: "原始貢獻度" },
                        { src: "/gm_img/gm_1_1_3.png", alt: "開發潛力" },
                        { src: "/gm_img/gm_1_1_4.png", alt: "已新增貢獻度" },
                      ].map((img, idx) => (
                        <img
                          key={idx}
                          src={img.src}
                    ...
                    {/* 第二列：新增貢獻度(月) + 分派狀態 */}
                    <div className="grid grid-cols-2 gap-4 justify-items-center items-start mx-auto max-w-[1240px] mt-2">
                      <img
                        src="/gm_img/gm_1_2_1.png"
                        alt="新增貢獻度（月）"
                        className="w-[630px] h-auto object-contain cursor-pointer align-top"
                        onClick={() => openImage("/gm_img/gm_1_2_1.png")}
                      />
                      <img
                        src="/gm_img/gm_1_2_2.png"
                        alt="分派狀態"
                        className="w-[570px] h-auto object-contain cursor-pointer align-top"
                        onClick={() => openImage("/gm_img/gm_1_2_2.png")}
                      />
                    </div>
                  </div>
                ) : (
                    <div className="flex flex-col space-y-10">
                      ...
                      <section className="flex flex-col items-center">
                        <div className="w-full max-w-[1240px]">
                          ...
                          <div className="grid grid-cols-3 gap-1 justify-items-center items-start">
                            {[
                              { src: "/gm_img/gm_2_1_1.png", alt: "開發潛力" },
                              { src: "/gm_img/gm_2_1_2.png", alt: "貢獻度達成率" },
                              { src: "/gm_img/gm_2_1_3.png", alt: "新增貢獻度" },
                            ].map((img, idx) => (
                              <img
                                key={idx}
                                src={img.src}
                      ...
                      <section className="flex flex-col items-center">
                        <div className="w-full max-w-[1240px]">
                          ...
                          <div className="grid grid-cols-3 gap-1 justify-items-center items-start">
                            {[
                              { src: "/gm_img/gm_2_2_1.png", alt: "名單處理建議" },
                              { src: "/gm_img/gm_2_2_2.png", alt: "客戶分配率" },
                              { src: "/gm_img/gm_2_2_3.png", alt: "處理進度" },
                            ].map((img, idx) => (
                              <img
                                key={idx}
                                src={img.src}
            ...
            <div className="bg-white rounded-2xl shadow-lg border border-gray-200 p-10">
              <div className="grid grid-cols-2 justify-center">
                <div className="flex flex-col items-left space-y-4">
                  ...
                  <div
                    className="border rounded-xl shadow-sm p-6 flex items-center justify-center"
          ...
          <MessageCenter
            ...
            handleSendToAreaManager={handleSendToAreaManager}
            dateRange={dateRange}
        ...
        <div className="flex flex-col border-r border-gray-200 pr-4">
          ...
          <div className="flex flex-col divide-y divide-gray-100 overflow-y-auto max-h-[600px]">
            {filteredMessages.length === 0 ? (
              <div className="text-gray-400 text-sm text-center py-10">無郵件內容</div>
            ) : (
              filteredMessages.map((msg: any) => (
                ...
                >
                  <div
                    ...
                    className="flex-1"
                  >
                    <h4 className="text-sm font-semibold text-gray-800">{msg.sender}</h4>
                    <h5 className="text-sm font-semibold text-gray-700 mt-1">{msg.subject}</h5>
                    <p className="text-xs text-gray-500 truncate mt-1">{msg.preview}</p>
                  </div>
                  <button
                    onClick={() => toggleStar(msg.id)}
                    className={`ml-2 ${msg.starred ? "text-orange-500" : "text-gray-400 hover:text-orange-400"}`}
                  >
          ...
          {selectedMessage ? (
            <>
              <div>
                ...
                <h5 className="text-base font-semibold text-gray-800 mb-3">{selectedMessage.subject}</h5>
                <p className="text-sm text-gray-600 leading-relaxed whitespace-pre-line">{selectedMessage.content}</p>

                {selectedMessage.replies?.length > 0 && (
                  <div className="mt-6 border-t border-gray-200 pt-3">
                    <h6 className="text-sm font-semibold text-gray-700 mb-2">回覆紀錄：</h6>
                    {selectedMessage.replies.map((r: any, idx: number) => (
                      <div key={idx} className="text-sm text-gray-600 mb-2">
                        <span className="font-semibold text-orange-600">{r.sender}：</span> {r.content}{" "}
              ...
              <div className="flex justify-between items-center mt-6 border-t pt-3 text-sm text-gray-600">
                <div className="flex space-x-6">
                  <button onClick={() => setShowReplyBox(true)} className="hover:text-orange-600 transition">
                    ↩ 回覆
                  </button>
                </div>
                <button onClick={() => handleDelete(selectedMessage.id)} className="hover:text-red-600 transition">
                  🗑 刪除
                </button>
              </div>
              ...
              {showReplyBox && (
                <div className="mt-4 border-t pt-4">
                  <div className="text-sm text-gray-600 mb-2 text-left">To: {selectedMessage.sender}</div>
                  <textarea
                    placeholder="輸入回覆內容..."
                    value={replyContent}
                    onChange={(e) => setReplyContent(e.target.value)}
                    rows={4}
                    className="w-full border border-gray-300 rounded-md px-3 py-2 text-sm focus:ring-1 focus:ring-orange-400"
                  ></textarea>
                  <div className="flex justify-end mt-3 space-x-3">
                    <button onClick={() => setShowReplyBox(false)} className="px-4 py-2 text-sm rounded-md border hover:bg-gray-100">
                      取消
                    </button>
                    <button
            ...
            <div className="text-gray-400 text-center py-40">請從左側選取一封郵件以查看內容</div>
          )}
        </div>
      </div>
    </div>
...
const SideIcon = ({ icon, label, onClick, active }: any) => (
  <button
    ...
    className={`relative flex items-center justify-center w-10 h-10 rounded-full transition-all group ${
      active ? "bg-orange-100 text-orange-600" : "text-gray-600 hover:bg-orange-50 hover:text-orange-500"
    }`}
  >
    {icon}
...
const TabButton = ({ label, active, onClick, width }: any) => (
  <button
    onClick={onClick}
    className={`text-sm font-semibold py-3 border-b-2 transition-colors ${width || "w-24"} ${
      active ? "border-orange-600 text-orange-600" : "border-transparent text-gray-500 hover:text-orange-500"
    }`}
  >
    {label}
  </button>
