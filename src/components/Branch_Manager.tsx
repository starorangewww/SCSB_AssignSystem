// =============================================
// Branch_Manager.tsx（完整版）
// 分行經理 Dashboard（含分頁分析 + 潛力名單情報站 + 訊息中心）
// =============================================
...
import potentialBranchData from "../data/potential_data.json";
...
// ---------------- 郵件邏輯 ----------------
    ...
    const loadMessages = () => {
      ...
      if (stored) {
        ...
        const related = allMessages.filter(
          (msg: any) =>
            msg.senderRole === "branch_manager" ||
            msg.recipientRole === "branch_manager" ||
            msg.ownerRole === "branch_manager"
        );
        setMessages(related);
      }
    };
  ...
  const currentUser = user?.username || "儲蓄部分行經理";
        ...
        {activeTab === "all" ? (
                  <div className="flex flex-col space-y-10">
                    ...
                    <div className="grid grid-cols-4 gap-4 justify-items-center items-center mx-auto max-w-[1240px]">
                      {[
                        { src: "/bgm_img/bgm_1_1_1.png", alt: "預測貢獻度" },
                        { src: "/bgm_img/bgm_1_1_2.png", alt: "原始貢獻度" },
                        { src: "/bgm_img/bgm_1_1_3.png", alt: "開發潛力" },
                        { src: "/bgm_img/bgm_1_1_4.png", alt: "已新增貢獻度" },
                      ].map((img, idx) => (
                        <img
                          key={idx}
                          src={img.src}
                    ...
                    <div className="grid grid-cols-2 gap-6 justify-items-center items-start mx-auto max-w-[1240px] mt-2">
                      <img
                        src="/bgm_img/bgm_1_2_1.png"
                        alt="新增貢獻度（月）"
                        className="w-[630px] h-auto object-contain cursor-pointer align-top"
                        onClick={() => openImage("/bgm_img/bgm_1_2_1.png")}
                      />
                      <img
                        src="/bgm_img/bgm_1_2_2.png"
                        alt="分派狀態"
                        className="w-[570px] h-auto object-contain cursor-pointer align-top"
                        onClick={() => openImage("/bgm_img/bgm_1_2_2.png")}
                      />
                    </div>
                  </div>
        ) : (
                    <div className="flex flex-col space-y-10">
                      ...
                      <section className="flex flex-col items-center">
                        <div className="w-full max-w-[1100px]">
                          ...
                          <div className="grid grid-cols-3 gap-8 justify-items-center items-start">
                            {[
                              { src: "/bgm_img/bgm_2_1_1.png", alt: "開發潛力" },
                              { src: "/bgm_img/bgm_2_1_2.png", alt: "貢獻度達成率" },
                              { src: "/bgm_img/bgm_2_1_3.png", alt: "新增貢獻度" },
                            ].map((img, idx) => (
                              <img
                                key={idx}
                                src={img.src}
                      ...
                      <section className="flex flex-col items-center">
                        <div className="w-full max-w-[1100px]">
                          ...
                          <div className="grid grid-cols-3 gap-8 justify-items-center items-start">
                            {[
                              { src: "/bgm_img/bgm_2_2_1.png", alt: "名單處理建議" },
                              { src: "/bgm_img/bgm_2_2_2.png", alt: "客戶分配率" },
                              { src: "/bgm_img/bgm_2_2_3.png", alt: "處理進度" },
                            ].map((img, idx) => (
                              <img
                                key={idx}
                                src={img.src}
...
const PotentialListDashboard: React.FC = () => {
  ...
  const [rmProgressData, setRmProgressData] = useState<{ [key: number]: string }>({});
  ...
  // ✅ 初始資料來源（若 localStorage 有更新過的版本，就用它）
  const [tableData, setTableData] = useState<any[]>(() => {
    const stored = localStorage.getItem("potential_data");
    return stored ? JSON.parse(stored) : potentialBranchData;
  });
  ...
  // ✅ 監聽 localStorage 更新事件（RM 收回 / 指派後即時同步）
  useEffect(() => {
    const stored = localStorage.getItem("rmProgressData");
    if (stored) setRmProgressData(JSON.parse(stored));

    const listener = () => {
      // 1️⃣ 同步 RM 進度
      const newStored = localStorage.getItem("rmProgressData");
      if (newStored) setRmProgressData(JSON.parse(newStored));

      // 2️⃣ 同步最新潛力名單資料
      const updatedPotential = localStorage.getItem("potential_data");
      if (updatedPotential) {
        const parsed = JSON.parse(updatedPotential);
        setTableData(parsed);
      }
    };
  ...
  const filteredData = currentData.filter(
    (item: any) =>
      item["客戶名稱"]?.includes(searchTerm) ||
      item["負責RM"]?.toString().includes(searchTerm)
  );

  // ✅ 三段式進度線（保留）
  const renderProgressLine = (status: string) => {
    const steps = ["未聯繫", "已聯繫", "已完成"];
    const activeIndex = steps.indexOf(status);

    return (
      <div className="flex flex-col items-center justify-center space-y-1 w-[200px] mx-auto">
        ...
        <div className="flex justify-between w-[170px] text-[11px] text-gray-600">
          {steps.map((step, i) => (
            <span
              key={i}
              className={`${i === activeIndex ? "text-orange-600 font-semibold" : ""}`}
            >
              {step}
            </span>
          ))}
    ...
    <div className="pt-4">
      ...
      <div className="flex justify-between items-center mb-4">
        ...
        <div className="relative">
          <input
            placeholder="搜尋RM或客戶名稱"
            value={searchTerm}
            onChange={(e) => setSearchTerm(e.target.value)}
            className="border border-gray-300 rounded-md px-10 py-1.5 text-sm focus:ring-1 focus:ring-orange-400"
          />
          <span className="absolute left-3 top-1.5 text-gray-400 text-base">🔍</span>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-lg border border-gray-200 p-6">
        <div className="overflow-x-auto scroll-smooth">
          <table className="min-w-[1550px] text-center text-sm border-collapse">
            ...
            <tbody>
              {filteredData.map((item: any) => {
                const progress =
                  rmProgressData[item.ID流水號] || item.處理進度 || "未聯繫";

                return (
                  <tr key={item.ID流水號} className="border-b hover:bg-orange-50 transition">
                    <td className="py-4 font-semibold text-gray-700">{item.ID流水號}</td>
                    <td>{item["自然人/法人"]}</td>
                    <td>{item["客戶名稱"]}</td>
                    <td>{item["潛力貢獻度預測"]}</td>

                    <td>
                      <div className="flex justify-between items-center w-[170px] mx-auto">
                        <span className="text-gray-700 text-sm">{item["處理建議"] || "—"}</span>
                        <button
                    ...
                    <td>{renderProgressLine(progress)}</td>

                    <td>
                      <button
                        className="text-blue-600 font-medium hover:underline"
                        onClick={() => handleOpen("rm", item)}
                      >
                        {item["負責RM"] && item["負責RM"] !== "" ? item["負責RM"] : "未分派"}
                      </button>
                    </td>
                      ...
                      <div className="flex justify-center items-center space-x-1">
                        <div className="h-1.5 w-16 bg-gray-200 rounded-full overflow-hidden">
                          <div
                            ...
                            style={{
                              width:
                                (parseFloat(item["開發成效追蹤"]) * 100 || 0).toFixed(0) + "%",
                            }}
                          ></div>
                        </div>
                        <span className="text-xs text-gray-600">
                          {(parseFloat(item["開發成效追蹤"]) * 100 || 0).toFixed(1)}%
                        </span>
                      </div>
                    </td>
      ...
      {showRM && <RMModal item={selectedItem} onClose={() => setShowRM(false)} />}
...
// ========================== RM Modal（可派發/收回） ==========================
const RMModal = ({ item, onClose }: any) => {
  const [selectedRM, setSelectedRM] = useState("");
  const [rmList, setRmList] = useState<string[]>([]);

  useEffect(() => {
    // ✅ 從 localStorage 或預設載入 RM 名單
    const storedAccounts = JSON.parse(localStorage.getItem("accounts") || "[]");

    // ✅ 過濾出 role = "rm" 的帳號
    const rms = storedAccounts
      .filter((acc: any) => acc.role === "rm")
      .map((acc: any) => acc.username);

    // 若 localStorage 沒有，就給預設值
    setRmList(rms.length ? rms : ["25836", "20311", "37494"]);
  }, []);

  const handleAssign = () => {
    if (!selectedRM) return alert("請先選擇要指派的 RM！");
    const updates = JSON.parse(localStorage.getItem("potential_updates") || "[]");

    // 🔹 檢查是否已有記錄
    const existing = updates.find((u: any) => u.id === item.ID流水號);
    if (existing) {
      existing.assignedRM = selectedRM;
    } else {
      updates.push({ id: item.ID流水號, assignedRM: selectedRM });
    }

    localStorage.setItem("potential_updates", JSON.stringify(updates));

    // ✅ 同步更新 potential_data.json 裡的「負責RM」欄位
    const allPotential = JSON.parse(localStorage.getItem("potential_data") || "[]");
    const updatedPotential = allPotential.map((d: any) =>
      d.ID流水號 === item.ID流水號
        ? { ...d, 負責RM: selectedRM === "未分派" ? "" : selectedRM } // ← 如果是未分派，就清空
        : d
    );
    localStorage.setItem("potential_data", JSON.stringify(updatedPotential));

    // ✅ 廣播更新事件，讓分行經理端畫面同步刷新
    window.dispatchEvent(new Event("storage"));

    alert(`✅ 已將 ${item.客戶名稱} 指派給 RM ${selectedRM}`);
    onClose();
  };
      ...
      <div className="bg-white rounded-xl shadow-xl w-[480px]">
        ...
        <div className="p-6 space-y-4 text-sm text-gray-700">
          ...
          <div className="flex items-center space-x-3">
            <label className="font-medium">重新指派：</label>
            <select
              value={selectedRM}
              onChange={(e) => setSelectedRM(e.target.value)}
              className="border border-gray-300 rounded-md px-2 py-1 focus:ring-1 focus:ring-orange-400"
            >
              <option value="">請選擇 RM</option>
              {rmList.map((rm) => (
